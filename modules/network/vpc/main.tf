...
resource "aws_flow_log" "this" {
  ...
  tags = merge(
    {
      "Name" = format("%s-${var.flow_log_destination_type}", var.name)
    },
    var.tags,
  )
}
...
resource "aws_db_subnet_group" "database" {
  count = var.create_vpc && length(var.private_subnets) > 0 && var.create_rds_subnet_group ? 1 : 0
...
resource "aws_default_security_group" "default_vpc" {
  ...
  tags = merge(
    {
      "Name" = format("SG-default-%s", var.default_vpc_name)
    },
    var.tags
  )
}
